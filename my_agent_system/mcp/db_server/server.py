...
print(f"DEBUG: MCP Server starting...")
...
def list_db_tables(dummy_param: str) -> dict:
    print(f"DEBUG: list_db_tables called with dummy_param: {dummy_param}")
    try:
        ...
        print(f"DEBUG: list_db_tables result: {result}")
        return result
    except sqlite3.Error as e:
        error_result = {"success": False, "message": f"Error listing tables: {e}", "tables": []}
        print(f"DEBUG: list_db_tables error: {error_result}")
        return error_result
        ...
        print(f"DEBUG: list_db_tables unexpected error: {error_result}")
        return error_result

def get_table_schema(table_name: str) -> dict:
    print(f"DEBUG: get_table_schema called with table_name: {table_name}")
    conn = get_db_connection()
    cursor = conn.cursor()
    ...
    print(f"DEBUG: get_table_schema result: {result}")
    return result

def query_db_table(table_name: str, columns: str = "*", condition: str = "1=1") -> list[dict]:
    print(f"DEBUG: query_db_table called with table_name: {table_name}, columns: {columns}, condition: {condition}")
    conn = get_db_connection()
    cursor = conn.cursor()
    ...
    try:
        ...
        print(f"DEBUG: query_db_table result count: {len(results)}")
        return results
    except sqlite3.Error as e:
        conn.close()
        error_msg = f"Error querying table '{table_name}': {e}"
        print(f"DEBUG: query_db_table error: {error_msg}")
        raise ValueError(error_msg)
    finally:
        conn.close()

def insert_data(table_name: str, data: dict) -> dict:
    print(f"DEBUG: insert_data called with table_name: {table_name}, data: {data}")
    if not data:
        error_result = {"success": False, "message": "No data provided for insertion."}
        print(f"DEBUG: insert_data error: {error_result}")
        return error_result
    ...
    try:
        ...
        print(f"DEBUG: insert_data result: {result}")
        return result
    except sqlite3.Error as e:
        conn.rollback()
        error_result = {
...
def delete_data(table_name: str, condition: str) -> dict:
    print(f"DEBUG: delete_data called with table_name: {table_name}, condition: {condition}")
    if not condition or not condition.strip():
        error_result = {
            "success": False,
            "message": "Deletion condition cannot be empty. This is a safety measure to prevent accidental deletion of all rows.",
        }
        print(f"DEBUG: delete_data error: {error_result}")
        return error_result
    ...
    try:
        ...
        print(f"DEBUG: delete_data result: {result}")
        return result
    except sqlite3.Error as e:
        conn.rollback()
        error_result = {
...
# MCP Server setup
print("DEBUG: Creating MCP Server instance for SQLite DB...")
...
print("DEBUG: Wrapping database utility functions as ADK FunctionTools...")
ADK_DB_TOOLS = {
...
async def list_mcp_tools() -> list[mcp_types.Tool]:
    print("DEBUG: MCP Server: Received list_tools request.")
    ...
    for tool_name, adk_tool_instance in ADK_DB_TOOLS.items():
        ...
        print(f"DEBUG: MCP Server: Advertising tool: {mcp_tool_schema.name}")
...
async def call_mcp_tool(name: str, arguments: dict) -> list[mcp_types.TextContent]:
    print(f"DEBUG: MCP Server: Received call_tool request for '{name}' with args: {arguments}")
    ...
    if name in ADK_DB_TOOLS:
        ...
        try:
            ...
            print(f"DEBUG: MCP Server: ADK tool '{name}' executed. Response: {adk_tool_response}")
            logging.info(f"MCP Server: ADK tool '{name}' executed. Response: {adk_tool_response}")
            response_text = json.dumps(adk_tool_response, indent=2)
            return [mcp_types.TextContent(type="text", text=response_text)]

        except Exception as e:
            print(f"DEBUG: MCP Server: Error executing ADK tool '{name}': {e}")
        ...
        print(f"DEBUG: MCP Server: Tool '{name}' not found/exposed by this server.")
...
async def run_mcp_stdio_server():
    print("DEBUG: MCP Stdio Server: Starting handshake with client...")
    logging.info("MCP Stdio Server: Starting handshake with client...")
    async with mcp.server.stdio.stdio_server() as (read_stream, write_stream):
        print("DEBUG: MCP Stdio Server: Running server...")
        ...
        print("DEBUG: MCP Stdio Server: Run loop finished or client disconnected.")
        logging.info("MCP Stdio Server: Run loop finished or client disconnected.")

if __name__ == "__main__":
    print("DEBUG: Launching SQLite DB MCP Server via stdio...")
    ...
    try:
        ...
        print(f"DEBUG: MCP Server (stdio) encountered an unhandled error: {e}")
        logging.critical(f"MCP Server (stdio) encountered an unhandled error: {e}", exc_info=True)
    finally:
        print("DEBUG: MCP Server (stdio) process exiting.")
